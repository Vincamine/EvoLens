...
while(<IA>){
	...
	`mkdir $inf[0]/2null` unless (-e "$inf[0]/2null");
	open OA, ">./$inf[0]/2null/codeml.ctl" or die;
	print OA <<RCD
seqfile = $pwd/$inf[0]_aln_dna.newphy
outfile = $pwd/$inf[0]/2null/$inf[0].rst
...
;
	...
	open OA, ">$inf[0]/2null/$inf[0].codeml.sh" or die;
	print OA "$pwd/codeml $pwd/$inf[0]/2null/codeml.ctl\n";
	close OA;

	print OT "cd $inf[0]/2null \nnohup sh $inf[0].codeml.sh 1>$inf[0].nohup.out 2>&1 &\ncd ../../\n";
}
close IA;
close OT;
